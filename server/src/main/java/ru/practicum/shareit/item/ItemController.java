...
public class ItemController {
    ...
    @GetMapping
    public List<ItemDtoWithBooking> getAllItemsByUserId(@RequestParam(defaultValue = "0") int from,
    ...
    @GetMapping("/search")
    public List<ItemDto> searchItem(@RequestParam(defaultValue = "0") int from,
                                    @RequestParam(defaultValue = "10") int size,
    ...
    public CommentResponseDto addComment(@PathVariable long id, @RequestBody CommentRequestDto commentRequestDto,
                                         @RequestHeader(USER_ID_HEADER) long userId) {
